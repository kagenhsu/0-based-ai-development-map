(() => {
  ...
  const acceptanceVersionGuideCard = (item) => {
    ...
    return `<article class="process-card requirements-card acceptance-version-card" id="step-${escapeHtml(item.step)}">
      ...
      <section class="version-operation-prompts" aria-labelledby="version-operation-prompts-title"><div class="requirements-section-head"><div><span>可直接使用</span><h3 id="version-operation-prompts-title">四項版本操作提示詞</h3></div><p>選擇需要的操作，填入版本號後直接複製使用。</p></div><div class="version-operation-prompt-grid">${guide.operationPrompts.map((entry) => `<article class="requirements-prompt version-operation-prompt"><div class="requirements-section-head"><div><span>${escapeHtml(entry.number)}</span><h4>${escapeHtml(entry.title)}</h4></div><button class="copy-requirements-prompt" type="button" data-copy-guide="acceptanceVersionGuide" data-copy-field="${escapeHtml(entry.field)}">複製提示詞</button></div><pre><code>${escapeHtml(guide[entry.field])}</code></pre></article>`).join("")}</div></section>
