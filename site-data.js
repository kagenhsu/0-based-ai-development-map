globalThis.AI_MAP_DATA = {
  title: "0 基礎 AI 開發地圖",
  subtitle: "從想法、需求到可驗收的網站、系統與 App。",
  fullMapUrl: "0-based-ai-development-map.html",
  homeStory: [
    "如果你跟我一樣，腦中偶爾會跳出一個 APP 的想法，我會帶你走完一整個流程。",
    "從我有一個想法，到它變成了一個在手機上能用的應用，全程不需要你寫一行程式碼，不需要你學任何程式語言，也不需要你花錢買任何軟體。",
    "你唯一需要做的就是對 AI 說出你的想法。我會帶你從專案準備、產品想法、需求文件、開發上線逐步推進，讓應用自己長出來。",
    "從選擇適合的智能體、大模型，建立工作空間，再到讓 AI 幫你理清需求、整理成功能清單，制定完整的產品需求文件、產品原型、UI 介面、技術開發文件與開發計畫，並進行開發、驗收、上線。",
    "你可以使用這個流程開發網頁、小程式、手機應用程式與電腦軟體。"
  ],
  prepGuide: {
    title: "第 0 步｜選一個 AI 開發助手、智能體",
    lead: "先選擇開發助手與大模型，再建立本機工作空間。後續文件、程式與測試都會放在同一個專案資料夾。",
    sections: [
      {
        title: "1｜選擇 AI 開發助手",
        kind: "assistant-comparison",
        paragraphs: [
          "不論是國外的 Claude Code、Codex，或 CodeBuddy、WorkBuddy、TRAE、豆包、千問等工具，都可以依任務選用。",
          "如果從來沒有用過，中國地區可先選擇安裝與操作較簡單的國產辦公智能體；台灣可使用 Claude Code 或 Codex。",
          "原稿的 WorkBody 經官方資料核對後校正為 WorkBuddy；「Coder」無法確認是指哪一項產品，先列為待確認。"
        ],
        bullets: [
          "安裝後即可使用，減少網路或服務限制",
          "不必手動配置複雜環境",
          "可切換不同的大模型",
          "可安裝各種技能與插件"
        ],
        warning: "注意：不要使用來源不明的第三方中介軟體，以免產生帳號、資料與付款風險。"
      },
      {
        title: "2｜替智能體選擇大模型",
        image: {
          src: "assets/model-routing-guide.png",
          alt: "高低模型搭配：高階模型決定方向，性價比模型負責大量執行，不確定時可使用自動模式。"
        },
        paragraphs: [
          "選完工具後，還要替智能體選一個負責思考的「大腦」——大模型。",
          "省錢的關鍵技巧是高低搭配：重要決策使用能力較高的旗艦模型，大量重複工作使用性價比較高的模型。",
          "如果不知道什麼時候切換，可先使用自動模式，讓智能體依任務選擇模型。"
        ]
      },
      {
        title: "3｜旗艦模型負責方向",
        kind: "model-task-matrix",
        paragraphs: [
          "設計整體架構、撰寫需求文件等全局性、核心、重要或複雜的任務，適合使用旗艦模型。",
          "這些任務工作量通常不大、Token 用量較少；先把方向與結構定好，後面的設計與開發才有意義。",
          "原稿模型例子：Claude Fable5、GPT 5.6 sol、GLM 5.2、Kimi K3、DeepSeek V4 Pro。"
        ],
        warning: "模型名稱、版本、價格與供應狀態待確認，使用前請查看各廠商官方資料。"
      },
      {
        title: "4｜性價比模型負責大量執行",
        image: {
          src: "assets/execution-model-guide.png",
          alt: "性價比模型從已確認規格、頁面程式碼、測試與修正，逐步產出可驗收版本。"
        },
        paragraphs: [
          "撰寫具體頁面程式碼等局部性、非核心、次要或較簡單的重複任務，適合使用單價較低的模型。",
          "這類任務本身不一定複雜，但工作量大、需要大量 Token，應優先控制成本。",
          "可從主流廠商的次旗艦或性價比產品中選擇；原稿列出的產品版本仍需向官方確認。"
        ]
      },
      {
        title: "5｜模型能力提醒",
        kind: "capability-checklist",
        paragraphs: [
          "原稿曾以 DeepSeek V4 與混元 3（Hy3）說明價格、圖片輸入與任務難度，但這些名稱與能力屬於待確認的原稿資訊，不作為目前版本的選型結論。",
          "實際選擇時，先依工作內容確認是否需要圖片輸入、本機檔案、網路搜尋、長期工作空間與企業資料保護，再用小任務測試。"
        ],
        warning: "免費方案、圖片輸入與模型能力會變動，以上內容須以官方最新說明為準。"
      },
      {
        title: "6｜建立本機工作空間",
        image: {
          src: "assets/workspace-setup-guide.png",
          alt: "建立專案資料夾、在智能體打開本機資料夾，集中存放文件、程式與測試，開始新任務時先選工作空間。"
        },
        paragraphs: [
          "在電腦上建立一個專案資料夾，例如「健身減肥日記」。",
          "在智能體中建立工作空間，選擇「打開本機資料夾」並指向這個資料夾。以後此專案的文件、程式與測試資料都放在這裡，查找與維護會更方便。",
          "開始新任務時，記得先選擇這個工作空間。"
        ]
      }
    ],
    assistantComparison: [
      { name: "Codex", form: "桌面 App／CLI／雲端", fit: "Windows 新手、完整專案開發", strengths: "讀寫專案、執行命令、圖片輸入、平行任務", setup: "登入 OpenAI；依版本確認方案與網路", note: "適合從規劃一路維護到測試", source: "OpenAI 官方", sourceUrl: "https://openai.com/codex/" },
      { name: "Claude Code", form: "終端機／IDE", fit: "複雜程式庫、開發者工作流", strengths: "理解程式庫、修改檔案、執行命令、MCP", setup: "安裝並登入；Windows 須符合官方環境需求", note: "較適合願意使用終端機者", source: "Anthropic 官方", sourceUrl: "https://docs.anthropic.com/en/docs/claude-code/getting-started" },
      { name: "CodeBuddy", form: "IDE／插件／CLI", fit: "零基礎、產品到部署流程", strengths: "需求、設計、開發、測試與部署整合", setup: "下載 IDE、插件或安裝 CLI", note: "使用前確認地區、帳號與資料政策", source: "騰訊雲官方", sourceUrl: "https://cloud.tencent.com/product/acc" },
      { name: "TRAE", form: "AI IDE", fit: "偏好圖形介面的新手與開發者", strengths: "程式問答、專案理解、生成、修復與優化", setup: "下載 IDE 並登入", note: "使用前確認地區與隱私設定", source: "TRAE 官方", sourceUrl: "https://docs.trae.cn/ide_chat" },
      { name: "千問／Qwen Code", form: "桌面 App／CLI／開源專案", fit: "需要開源、多模型或進階擴充", strengths: "Skills、Hooks、MCP、沙箱、工作樹", setup: "桌面安裝或 CLI 配置模型驗證", note: "進階功能需要較多設定", source: "Qwen 官方", sourceUrl: "https://github.com/QwenLM/qwen-code" },
      { name: "WorkBuddy", form: "AI 原生桌面智能體工作台", fit: "辦公流程、日常工作與開發工具切換", strengths: "官方定位為桌面智能體工作台", setup: "依騰訊雲帳號與方案啟用", note: "開發工作可搭配 CodeBuddy", source: "騰訊雲官方", sourceUrl: "https://cloud.tencent.com/announce/detail/2270" },
      { name: "豆包", form: "對話式 AI 應用", fit: "想法整理、問答與內容草稿", strengths: "以 AI 對話為核心的文字、語音等互動", setup: "使用官方 App 或網站", note: "本機專案操作能力須依實際版本確認", source: "豆包官方", sourceUrl: "https://www.doubao.com/legal/instructions" },
      { name: "Coder", form: "名稱待確認", fit: "待確認", strengths: "同名產品很多，無法安全比較", setup: "請先提供官方網址或完整產品名", note: "確認前不建議下載或付款", source: "待確認", sourceUrl: "" }
    ],
    modelTaskMatrix: [
      { task: "產品方向與範圍", tier: "高階模型", reason: "會影響整體目標與後續成本", examples: "需求訪談、功能取捨、PRD" },
      { task: "系統架構與資料安全", tier: "高階模型", reason: "跨模組且錯誤代價較高", examples: "TRD、權限、資料模型、API" },
      { task: "複雜除錯與程式審查", tier: "高階模型", reason: "需要跨檔案推理與找根因", examples: "疑難錯誤、安全與回歸風險" },
      { task: "依規格製作頁面", tier: "性價比模型", reason: "方向已確認，主要是大量執行", examples: "HTML、CSS、元件與文案套版" },
      { task: "測試案例與重複修正", tier: "性價比模型", reason: "工作量大且可用驗收條件核對", examples: "單元測試、格式修正、批次調整" },
      { task: "跨模組或規格不清的任務", tier: "自動／混合", reason: "先由高階模型拆解，再交付執行", examples: "大型功能、長鏈路改版" }
    ],
    capabilityChecklist: [
      { use: "整理文字需求", need: "文字理解、長上下文", check: "貼一小段需求，確認能否正確摘要", level: "基本" },
      { use: "看圖片或截圖", need: "圖片輸入／視覺能力", check: "上傳測試截圖，請它指出畫面元素", level: "必查" },
      { use: "修改本機專案", need: "檔案與命令工具權限", check: "用測試資料夾確認讀檔、改檔與執行", level: "必查" },
      { use: "查最新官方資料", need: "網路搜尋與來源引用", check: "要求附官方連結與查詢日期", level: "必查" },
      { use: "長期維護同一專案", need: "工作空間、規則與版本管理", check: "關閉重開後確認仍能找到專案規則", level: "必查" },
      { use: "處理公司敏感資料", need: "權限、隱私與企業管理", check: "先查官方資料政策；不要上傳機密原文", level: "高風險" }
    ]
  },
  requirementsGuide: {
    title: "第一步：需求收集（做什麼）",
    lead: "好，準備工作完成。接下來先不要急著開發，而是把腦中的模糊想法問清楚。",
    intro: [
      "大多數人做應用程式最大的問題不是技術，而是說不清楚要什麼。",
      "你可能只能先說出「我想做一個記錄健身減肥的應用」，但還不知道要有哪些功能、操作流程與畫面。反過來要求自己一開始就想清楚全部需求，也不現實。",
      "最好的方法不是一次把所有答案告訴 AI，而是先說出大概方向，讓 AI 像產品經理一樣，每次問一個問題，逐步挖出真正需求。"
    ],
    method: [
      "這裡使用 grill-me skill，以一問一答方式進行需求訪談。",
      "當回答太模糊時，AI 會繼續追問；資料足夠後，再整理成完整的使用者需求文件交給你確認。",
      "若使用的工具支援語音輸入，也可以直接用說的回答，就像聊天一樣完成訪談。"
    ],
    prompt: `/codex-with-chatgpt
/grill me

請用一問一答收集我想要做的應用的需求
每次只輸出1個問題，等我回復再問下一個，禁止一次性輸出大量問題
提問覆蓋：目標用戶、真實痛點、使用場景、需要的功能、不要的功能、運行設備、參考產品、特殊限制
我回答模糊就追問細節，訊息足夠之後，輸出一份結構畫完整需求文檔給我確認

我準備做一個應用：__________________
我目前的初步想法：__________________

現在直接開始`,
    example: {
      app: "健身減肥日記手機網頁應用",
      idea: "記錄我每天的運動健身狀況、日常飲食、體能與體重，並根據這些資訊給我相關建議。",
      questions: [
        "工具是自己用，還是也給別人用？",
        "什麼時候記錄？",
        "要解決什麼痛點？",
        "需要記錄哪些資訊？",
        "資料要記錄到多細？",
        "要提供什麼建議？",
        "哪些功能不需要？",
        "手機上如何呈現？",
        "有哪些參考產品？",
        "資料要存放在哪裡？",
        "建議如何產生？",
        "希望使用什麼視覺風格？"
      ]
    },
    closing: "完成問答後，原本模糊的想法會被整理成一份完整的使用者需求文件，作為下一步功能清單的輸入。"
  },
  featureGuide: {
    title: "第二步：功能清單（應用功能）",
    lead: "需求聊清楚後，下一步讓 AI 幫你輸出一份完整的功能清單。",
    intro: [
      "功能清單是整個產品的地基。這一步先確認產品要具備哪些能力，再進入 UI、互動與程式開發。",
      "關鍵是要求 AI 依照「模組 → 功能 → 子功能」三層拆開，並標示核心必做、次要迭代與未來可選功能。",
      "現在多花時間逐條檢查，通常比進入開發後反覆返工更省時間。"
    ],
    hierarchy: [
      { level: "一級模組", example: "運動紀錄", description: "產品主要能力區域" },
      { level: "二級功能", example: "運動紀錄輸入", description: "模組底下的具體功能" },
      { level: "三級子功能", example: "開始時間、運動類型", description: "功能需要包含的細項" }
    ],
    prompt: `/codex-with-chatgpt
/grill me

要求：
案這模組化拆分，分為一級模組、二級功能、三級子功能；
區分：核心必做功能、次要迭代功能、未來可選用規劃功能，
用標籤標註；
每條功能寫簡短功能描述，說明這個功能決什麼使用者問題；
不要寫UI交互細節，只輸出功能能力；
輸出格式用Markdown列表，清晰分層，不要大段文字`,
review: [
      "逐條閱讀 AI 產出的功能清單，確認每項功能是否真的解決使用者問題。",
      "發現不正確或遺漏時，可以截圖並圈出問題，直接告訴 AI 要補充或修正的內容。",
      "修正後要求 AI 重新輸出完整功能清單，不要只回覆局部修改。"
    ],
    example: {
      issues: [
        { feature: "運動紀錄輸入", missing: "缺少運動開始時間" },
        { feature: "飲食紀錄輸入", missing: "缺少用餐時間" }
      ],
      correction: "運動記錄缺少運動開始時間；飲食記錄缺少用餐時間。請更新並重新給出完整的功能清單。"
    },
    detailCompletionPrompt: `/codex-with-chatgpt
/grill me

你現在是資深產品經理，請根據我提供的需求文件、目前功能清單、截圖與補充要求，協助我補齊功能清單的細節。

要補充的一級模組：______________________________
要補充的二級功能：______________________________
目前已有內容：______________________________
目前缺少的細節：______________________________
我希望補充成：______________________________
優先級（核心必做／次要迭代／未來可選）：______________________________
必須保留的內容：______________________________
相關截圖或補充資料：______________________________

執行規則：
1.先核對使用者需求與目前功能清單，確認要補充的位置、缺漏內容與預期結果
2.資訊不足時，每次只問我 1 個最關鍵的問題，等我回答後再繼續
3.依照「一級模組 → 二級功能 → 三級子功能」補充，維持原有層級與命名一致
4.每條新增或修改的功能都要附簡短描述，說明解決哪一個使用者問題
5.只描述產品功能能力，不加入 UI 版面、按鈕位置或視覺設計細節
6.檢查補充內容是否與其他模組重複、衝突或產生前置依賴，必要時列出需要同步調整的項目
7.依照實際優先級標記「核心必做、次要迭代、未來可選」，不要自行提高範圍
8.無法從現有資料確認的產品規則標記【需產品確認】，不要自行腦補
9.先輸出修改摘要，再重新輸出整合後的完整功能清單，不要只提供局部片段

現在先檢查我提供的資料是否足夠；足夠就直接補充，不足就從最關鍵的 1 個問題開始。`,
    closing: "功能清單確認完成後，會得到依模組、功能與子功能分層，並標示優先順序的完整產品能力清單。"
  },
  prdGuide: {
    title: "第三步：產品需求文檔（PRD）",
    lead: "功能清單告訴你「要有什麼」，PRD 則進一步定義每項功能「怎麼運作」。",
    intro: [
      "接下來要把已確認的功能清單寫成產品需求文檔，讓設計、開發與測試都能依照同一套規則工作。",
      "你不需要先學會撰寫這類文件，可以把功能清單、需求描述與使用場景交給 AI，請它整理成標準 PRD 草稿。"
    ],
    comparison: [
      { name: "功能清單", focus: "要有什麼", description: "確認模組、功能、子功能與優先順序" },
      { name: "產品需求文檔 PRD", focus: "怎麼運作", description: "定義輸入、操作、輸出、規則、異常與權限" }
    ],
    questions: [
      "這個按鈕點下去會發生什麼？",
      "資料要怎麼保存？",
      "輸入錯誤時如何提示？",
      "沒有網路時要怎麼處理？"
    ],
    prompt: `/codex-with-chatgpt
/grill me

你現在是資深互聯網產品經理，擅長把零散的功能清單、需求點子、零散需求，
轉化為結構標準、邏輯完整、可直接交付開發/測試的正是PRD文檔
請嚴格照以下規則執行：
1.輸入內容：我將提供零散的功能清單、修求描述、使用場景
2.輸出格式：標準產品PRD文檔，結構完整、條理清晰、無口語化表述、邏輯閉環；
3.必須包含模組：文檔概述(背景、目的、適用範圍)、需求受眾、產品目標、功能詳細敘述需求、業務流程、交互規則、異常場景、權限說明、非功能需求、上線(上架)約束、需求排期、備註；
4.功能需求部分：逐條拆解每個功能，明確輸入、操作流程、輸出、規則、限制條件，覆蓋正常場景+所有異常場景；
5.自動補全缺失訊息：若我提供的功能清單訊息不全，請基於主流產品邏輯合理補充，不遺漏核心業務規則，不隨意刪減需求；
6.語言需求：正式、專業、落地，符合國內(台灣)互聯網團隊開發交付標準，無需沉餘廢話，可直接用於評審、開發、測試`,
    review: [
      "收到 PRD 後先快速閱讀，確認產品方向、功能規則與使用情境是否符合你的需求。",
      "發現問題時可截圖並圈出位置，告訴 AI 要修改的規則。",
      "要求 AI 更新相關功能、資料與非功能需求，最後重新輸出完整 PRD。"
    ],
    correctionPrompt: "食物描述要能加入食物圖片，添加圖片非必填，並更新給出完整的文檔",
    detailRevisionPrompt: `/codex-with-chatgpt
/grill me

你現在是資深產品經理，請根據我提供的 PRD、截圖與修改要求，協助我精確修改產品需求文檔的細節。

我要修改的位置或章節：______________________________
目前內容或問題：______________________________
我要調整成：______________________________
必須保留的內容：______________________________
相關截圖或補充資料：______________________________

執行規則：
1.先確認修改位置、目前規則、預期結果與驗收方式；資訊不足時每次只問我 1 個問題，等我回答後再繼續
2.只修改我指定的需求，不自行增加新功能，也不要刪除未要求變更的內容
3.檢查這次修改是否影響相關功能、操作流程、資料欄位、輸入與輸出、權限、異常處理、非功能需求及驗收條件
4.遇到無法從現有資料確認的業務規則，標記【●待產品／業務（功能）確認】，不要自行腦補
5.先輸出修改摘要，列出「修改前、修改後、影響章節、需要同步調整的內容」
6.完成修改後重新輸出整合後的完整 PRD，不要只提供局部片段
7.在文件變更紀錄中加入本次修改內容、版本號與日期，保留可追蹤性
8.最後提供驗收檢查清單，讓我確認修改內容與關聯規則是否一致

現在先檢查我提供的資料是否足夠；足夠就直接修改，不足就從最關鍵的 1 個問題開始。`,
    correctionResult: "AI 除了更新食物描述功能，也要同步檢查圖片資料、容量、效能與相關異常處理是否需要調整。",
    note: "如果你完全不懂技術，可以先略過技術細節；這份文件主要交給後續設計、開發與測試使用，但功能方向與使用者需求仍要由你確認。",
    closing: "PRD 確認後，會得到可直接用於評審、設計、開發與測試的完整產品需求文檔。"
  },
  prototypeGuide: {
    title: "第四步：原型設計（頁面交互）",
    lead: "PRD 是文字，原型則是可以直接點擊、操作與驗收的網頁。",
    intro: [
      "原型雖然還沒有連接真實資料，但可以先完整走過頁面、導覽、彈窗與表單流程。",
      "在撰寫正式程式碼前先確認操作方式，可以更早看見想法與實際使用之間的偏差，降低後續開發返工。"
    ],
    journey: [
      { number: "01", title: "閱讀 PRD", description: "確認頁面、功能與業務規則" },
      { number: "02", title: "產生可點擊原型", description: "建立頁面、彈窗、表單與導覽" },
      { number: "03", title: "實際操作驗收", description: "逐步點擊並截圖回報問題" }
    ],
    prompt: `你是產品原型專家，嚴格依據PRD輸出高保真產品原型設計
輸出要求：
梳理全部頁面、彈窗、抽屜、浮層；每個頁面寫清楚佈局結構
每一個組件說明清楚：位置、文字、按鈕文案、組件類型、默認狀態
交互動實現寫清楚：點擊哪個按鈕→發生甚麼，跳轉去哪里，彈窗展示甚麼，表單題驗題示文案、彈窗、全局組件(導覽列、底部tab、側邊選單)
不做UI視覺美化描述，只講原型結構，方便直接照著拖曳組件畫圖
PRD資訊不足時標記【需產品確認】，不腦補需求`,
    acceptance: [
      "點擊導覽列，確認能切換不同功能。",
      "點擊「新增運動記錄」，確認表單會出現並能填寫、儲存。",
      "回到清單頁，確認新增的記錄已經出現。",
      "發現不正確的地方時截圖並標示位置，再告訴 AI 如何調整。"
    ],
    changeExample: [
      "添加飲食記錄時，需要增加卡路里攝取量。",
      "設定頁不需要使用幫助和隱私說明。",
      "首頁增加卡路里模組，包含基礎消耗、運動消耗與攝取量。",
      "計算今日卡路里缺口：攝取量－基礎消耗－運動消耗。"
    ],
    detailRevisionPrompt: `/codex-with-chatgpt
/grill me

你現在是產品原型專家，請根據我提供的 PRD、目前原型、截圖與修改要求，直接協助我修改產品原型的操作細節。

我要修改的頁面或元件：______________________________
目前的問題：______________________________
我要調整成：______________________________
預期操作流程：______________________________
必須保留的內容：______________________________
相關截圖或補充資料：______________________________
需要的示範資料（選填）：______________________________

執行規則：
1.先核對 PRD 與目前原型，確認修改位置、目前狀態、預期結果與驗收方式
2.資訊不足時，每次只問我 1 個最關鍵的問題，等我回答後再繼續
3.只修改我指定的內容，不自行增加產品功能，也不要刪除未要求變更的頁面或互動
4.清楚處理點擊、跳轉、彈窗、表單驗證、導覽，以及預設、載入中、空白、錯誤、成功等必要狀態
5.檢查共用元件、前後頁面、導覽與相關操作流程是否需要同步調整，避免只修單一畫面
6.如果修改會改變產品功能、資料欄位、計算規則、權限或異常處理，先列出影響；無法確認的規則標記【需產品確認】，不要自行腦補
7.有填寫示範資料時，將資料加入原型，讓我可以直接操作與查看各種狀態
8.修改完成後，提供「修改前、修改後、影響範圍、測試結果」摘要，並交付可直接點擊驗收的完整原型

現在先檢查我提供的資料是否足夠；足夠就直接修改，不足就從最關鍵的 1 個問題開始。`,
    impact: [
      "原型調整可能同時改變產品功能、分析建議、趨勢內容，以及底層資料保存與計算邏輯。",
      "修改完成後，不只要更新畫面，也要系統性同步功能清單、PRD 與原型，避免文件版本互相矛盾。"
    ],
    syncPrompt: "根據當前調整，依次系統性的更新功能清單、產品需求文檔PRD、原型到最新版本",
    closing: "持續操作、截圖與修正，直到原型流程符合需求；文件同步更新後，再進入 UI 視覺設計。"
  },
  uiGuide: {
    title: "第五步：UI 設計（好看）",
    lead: "原型確認後，再替產品建立一致、清楚且可實作的視覺介面。",
    intro: [
      "可以直接描述想要的設計風格，也可以提供參考截圖，請 AI 依照畫面層級、留白、卡片、字體與配色方向設計。",
      "只有風格關鍵字時，設計結果可能仍較普通；搭配參考圖與 UI 設計 Skill，通常能更明確地提升視覺完成度。"
    ],
    formula: [
      { number: "01", title: "好關鍵字", description: "說清楚風格、產品氣質與使用情境" },
      { number: "02", title: "好參考圖", description: "提供喜歡的版面、配色與元件截圖" },
      { number: "03", title: "好 Skill", description: "使用 /ui-ux-pro-max 強化層級與細節" }
    ],
    prompt: `/ui-ux-pro-max

你是資深前端UI工程師，專業UI設計師的AI團隊主管
基於產品需求與原型，輸出完整UI設計
頁面需求請嚴格遵守產品需求，頁面佈局與互動都要遵守先前的產品需求和原型
主題風格：____________________________________________________________
配色：____________________________________________________________`,
referenceTips: [
      "提供 2～4 張最接近目標風格的畫面，並指出要參考的是版面、配色、元件或留白。",
      "不要只說『照著做』，要標明哪些元素需要保留、哪些只作為風格參考。",
      "介面完成後逐頁操作，檢查導覽、按鈕、表單、彈窗與不同螢幕尺寸。"
    ],
    progress: [
      "第一層：只提供風格關鍵字，先建立基本視覺方向。",
      "第二層：加入 iOS 或其他產品截圖，校準卡片、留白與元件細節。",
      "第三層：使用 /ui-ux-pro-max 重新檢查資訊層級、可讀性、響應式與操作體驗。"
    ],
    maturityPrompt: `/ui-ux-pro-max

你是資深 UI／UX 設計審查、前端調整與測試主管。
請基於目前已確認的產品需求、PRD、產品原型、UI 設計與現有頁面，進行設計成熟度檢查、調整與測試。

執行原則：
1.保留我目前選定的主題風格與配色，不替我指定新的風格、色調或品牌方向
2.除非我明確要求，否則不要因個人審美重新設計整個介面，只修正有證據的可用性、一致性、響應式或操作問題
3.先檢查再修改；每個問題要指出頁面、元件、觸發方式、實際結果、預期結果與影響程度
4.檢查桌面、平板與手機尺寸，確認內容不溢位、不重疊、不被固定頁首或側邊欄遮住
5.檢查資訊層級、間距、對齊、文字可讀性、按鈕辨識、表單提示與元件一致性
6.檢查一般、滑入、焦點、停用、載入、空白、錯誤、成功等必要狀態，以及鍵盤操作與基本無障礙
7.實際操作導覽列、側邊欄、按鈕、表單、彈窗、抽屜、篩選、複製與返回頂端等互動
8.若對比度或辨識度不足，先標示問題與影響；沒有我的確認，不要自行改變既定配色方向
9.修改時採最小必要變更，保留既有功能、內容與互動規則，不增加產品需求之外的功能
10.每次修改後重新測試受影響頁面與三種螢幕尺寸，確認問題已修正且沒有產生新的回歸問題

輸出格式：
- 設計成熟度摘要
- 問題清單：頁面／元件、問題、影響、優先級、修改位置、驗收方式
- 已完成修改：修改檔案、調整內容
- 測試結果：尺寸、操作路徑、通過／失敗、證據
- 尚待我確認的設計選擇

本次要檢查的頁面或網址：______________________________
目前發現的問題或截圖說明：______________________________
必須保留的設計與功能：______________________________

現在開始檢查；如果我已明確指出要修改的位置，請直接調整並完成測試。`,
    syncPrompt: "請依據剛才的調整，幫我全面系同更新整理：功能清單、產品需求文件PRD、產品原型、UI設計",
    closing: "UI 驗收與文件同步完成後，功能、需求、原型與視覺設計即可一起定稿，交給後續技術文件與開發階段。"
  },
  trdGuide: {
    title: "第六步：技術開發文檔（施工圖紙）",
    lead: "TRD 從系統架構角度，先規劃資料、接口與模組如何連動，再開始撰寫程式碼。",
    intro: [
      "蓋房子不會沒有施工圖就開工，開發應用程式也一樣。PRD、原型與 UI 確認後，先把可落地的技術規則整理清楚。",
      "這份文件用來讓前端、後端、測試與運維理解同一套架構、資料、異常、權限、安全及驗收要求。"
    ],
    flow: [
      { number: "01", title: "產品文件", description: "PRD、原型與 UI 定稿" },
      { number: "02", title: "TRD 施工圖", description: "拆解架構、資料、接口與風險" },
      { number: "03", title: "進入開發", description: "依技術規則實作與測試" }
    ],
    outline: [
      { title: "文檔訊息", description: "版本、目標、業務邊界與干係人" },
      { title: "業務分析", description: "核心流程、使用場景與業務規則" },
      { title: "功能模組技術分解", description: "逐條拆解前後端、校驗與異常處理" },
      { title: "數據模型設計", description: "資料表、核心欄位、流轉與保留策略" },
      { title: "API 接口需求清單", description: "接口用途、請求方式與後續文件需求" },
      { title: "非功能性需求", description: "效能、權限、日誌、安全與相容性" },
      { title: "外部與內部依賴", description: "服務、第三方元件與下游影響" },
      { title: "風險評估與限制條件", description: "技術、業務、效能風險與規避方式" },
      { title: "待確認問題清單", description: "集中列出 PRD 缺失與模糊規則" },
      { title: "測試校驗要點", description: "功能、邊界、異常與資料校驗重點" }
    ],
    prompt: `你是資深系統框架師主管，負責把產品PRD轉化為開發可以用的技術需求文檔TPD。根據完整產品需求文檔PRD輸出技術需求文檔TRD，包含業務背景、用戶故事、功能說明、交互規則、約束條件。忠實原始PRD，不新增產品功能。業務描述模糊、缺失約束全部標記
【●待產品/業務(功能)確認】，禁止自行腦捕是業務(功能)規則。然後統一匯總出問題，並給可選擇的解決文案提供我選擇確認

#輸出文檔結構（嚴格下面章節輸出Markdown格式TRD）
1文檔訊息
1.1文檔版本、變更紀錄
1.2業務背景與建設目標
1.3業務邊界(那些做、那些不做)
1.4干係人：產品、後端、前端、測試、運維

2業務分析
2.1核心業務流程圖(文本Mermaid流程圖程式碼)
2.2用戶場景梳理
2.3業務約束與業務規範匯總

3功能模組技術分解【核心章節】
對PRD每一條產品需求，逐條翻譯成技術需求：
產品需求原文引用
前端技術需求
後端技術需求
輸入校驗規則
全量異常場景&錯誤碼定義說明
前端交互異常處理

4數據模型設計
4.1涉及數據表：新增表/修改已有表
4.2核心字段說明：字段名、類型、約束、業務含意；不輸出完整建表SQL(依照使用者史常用情況而定)
4.3數據流轉說明：數據如何產生、更新、刪除；數據保留策略

5API接口需求清單
輸出接口列表：接口名稱、接口用途、請求方式、簡要提供參閱；不需要完整json示例，複雜接口可以標註需要後續輸出接口文檔

6非功能性需求
6.1性能需求：响應時間、開發量、QPS預期、大數據分頁處理
6.2權限需求：角色、數據權限、功能權限
6.3埋點需求：需求埋點事件清單、埋點上報參數
6.4日誌需求：關鍵操作日志、錯誤日誌
6.5安全需求：參數防竄改、防重複提交、敏感數據脫敏
6.6間容性：終端、瀏覽器、版本約束
6.7運維：告警點、監控指標

7外部&內部依賴
依賴內部服務模組
依賴第三方接口/組件
下游影響：那些有業務會受本次改動影響

8風險評估與限制條件
技術風險、業務風險、性能風險，給出風險說明和建議規避方式

9待確認問題清單
把PRD缺失、描述模糊的點全部匯總在這裡，統一標記待確認

10測試校驗要點
給測試同學參考：功能用例重點、邊界用例、異常用例、數據校驗點

輸出格式約束
使用標準Markdow，標題層級# ## ###，表格盡量用markdown表格；
語言是研發視角，去掉產品營消話術，只保留可落地技術描述；
Mermaid流程圖直接輸出代碼快；
不要輸出PRD原文大段複製，要做拆解轉化；
不要做方案選型(比如不能指定用Redis/Mysql，出非PRD明確給出)`,
    audiences: [
      { title: "懂技術開發", description: "閱讀並確認架構、資料、接口、權限、安全、風險與測試要點。" },
      { title: "一般使用者", description: "可以略過技術細節，優先處理標記為待確認的產品或業務問題，並採用文件提供的可選解決方式。" }
    ],
    note: "原文同時使用 TPD 與 TRD，正式使用前請確認是否統一為 TRD；其餘原文內容保持不變。",
    closing: "TRD 確認後，開發團隊會取得可落地的施工圖紙，接著才能安全地拆解開發計畫並開始實作。"
  },
  developmentPlanGuide: {
    title: "第七步：開發計畫（任務拆解）",
    lead: "把一個複雜的開發任務拆成可控制、可驗收的小任務，再依順序逐步完成。",
    intro: [
      "TRD 完成後，不要直接把整個專案一次交給 AI 開發。先整理迭代、里程碑、前後端與測試任務，才能在每個節點確認方向。",
      "任務拆小後，也能把大量執行工作交給性價比較高的模型，重要決策則保留給能力較高的模型。"
    ],
    benefits: [
      { number: "01", title: "每一步都可控", description: "把一個大任務拆成約 20 個小任務，每完成一項就檢查交付結果，避免做到一半才發現方向錯誤。" },
      { number: "02", title: "降低模型成本", description: "小型、重複的執行任務可使用性價比較高的模型；保留高階模型處理架構、風險與重要決策。" }
    ],
    flow: [
      { number: "01", title: "讀取 PRD／TRD", description: "確認產品與技術範圍" },
      { number: "02", title: "設定迭代與里程碑", description: "決定週期與交付節點" },
      { number: "03", title: "拆解開發任務", description: "標註角色、依賴與驗收" },
      { number: "04", title: "依序執行", description: "完成一項、驗收一項" }
    ],
    prompt: `你是系統架構師和研發專案經理，根據PRD和TRD輸出一份開發實施計劃
輸出內容：
制定迭代計畫、迭代週期、里程碑；
任務分解(前端、後端、測試)，標註前置依賴；
輸出使用Markdown表格，簡潔實務，適合迭帶評審`,
    tableRows: [
      { iteration: "迭代 1", milestone: "基礎流程可操作", role: "前端", task: "建立主要頁面與導覽流程", dependency: "UI 設計定稿", acceptance: "主要頁面可進入與切換" },
      { iteration: "迭代 1", milestone: "核心資料可保存", role: "後端", task: "建立核心資料寫入與查詢", dependency: "TRD 資料模型", acceptance: "正常與異常資料皆有明確回應" },
      { iteration: "迭代 2", milestone: "核心功能可驗收", role: "前後端", task: "串接主要功能與狀態處理", dependency: "前後端基礎任務", acceptance: "核心使用流程可完整走通" },
      { iteration: "迭代 2", milestone: "版本可交付", role: "測試", task: "執行功能、邊界與異常測試", dependency: "核心功能完成", acceptance: "阻擋問題修正並通過複驗" }
    ],
    closing: "把開發計畫確認後，就能照著依賴順序逐項執行；每個任務都有負責角色、前置條件與驗收結果。"
  },
  productDevelopmentGuide: {
    title: "第八步：產品開發（編碼）",
    lead: "按照開發實施計畫表持續開發，不因完成單一任務或一次測試而中斷。",
    intro: [
      "這個階段開始把 PRD、原型、UI、TRD 與開發計畫轉成可運作的程式。開工前先做文件交叉校驗，發現衝突或缺失時先確認，不要直接猜測。",
      "確認後，AI 應從開發實施計畫表的第一個未完成任務開始，依前置關係持續執行。每完成一項就測試、修正、記錄進度並接續下一項，直到全部完成或遇到必須由你決定的阻礙。",
      "為避免工作中斷，請在專案內保存開發進度與測試結果。下一次對話可先讀取進度檔、開發計畫與目前程式狀態，再從中斷點續作。"
    ],
    skill: {
      command: "/superpower",
      name: "Superpower skill",
      description: "用於強化規劃、執行與驗證流程；使用前請先確認目前工具已安裝並支援此呼叫名稱。"
    },
    flow: [
      { number: "01", title: "讀取計畫與進度", description: "找出下一個未完成任務" },
      { number: "02", title: "執行任務", description: "依前置關係完成編碼" },
      { number: "03", title: "測試與修正", description: "失敗就定位原因並複測" },
      { number: "04", title: "記錄檢查點", description: "更新狀態、結果與下一步" },
      { number: "05", title: "自動接續", description: "沒有阻礙就繼續下一項" }
    ],
    rules: [
      "先校驗 PRD、原型、UI、TRD 與開發計畫",
      "嚴格依照開發實施計畫的順序、優先級與前置關係",
      "每項任務都要完成編碼、測試、失敗修正與進度記錄",
      "完成單一任務後直接接續，不把階段性回報當成停止點",
      "只有需求衝突、缺少必要資料或高風險操作才暫停提問"
    ],
    prompt: `/superpower

你是開發+測試全端工程師，依照產品需求文檔PRD、產品原型、UI設計、TRD、開發計畫做開發與測試工作
1.先對文檔交叉進行校驗，輸出衝突、缺失疑問清單，確認後開工
2.嚴格按照開發計畫模組順序&優先開發，不私自增加功能
3.每個模組同時輸出：數據庫、業務接口代碼、單完測試代碼、接口測試用例、自側核對清單
4.測試用例對齊PRD驗收標準，可直接用於提測
5.訊息不足直接提問，禁止腦補需求

持續執行規則：
6.先讀取開發實施計畫表與目前開發進度，找出第一個尚未完成且前置條件已滿足的任務
7.每完成一項任務，立即執行該項測試；測試失敗時先定位原因、做最小必要修正並重新測試，直到通過或確認為真正阻礙
8.每項任務完成後更新開發進度紀錄，至少寫明：任務編號、完成內容、修改檔案、測試指令、測試結果、未解問題、下一個任務
9.如果沒有需要我決定的需求衝突、必要資料缺失、權限問題或高風險操作，不要停在回報或詢問是否繼續；請直接依計畫執行下一個任務
10.持續循環「讀取下一項→開發→測試→失敗修正→複測→記錄→下一項」，直到開發實施計畫全部完成
11.不要刪除既有測試、降低驗收標準或用跳過測試的方式製造通過結果
12.不得自行部署到正式環境、寫入正式資料或執行不可逆操作；遇到這些情況先停下並說明需要我確認的內容

現在先告訴我目前讀到的開發計畫、已完成項目、下一個任務與預計執行的測試，然後開始持續執行。`,
    continuationPrompt: `/superpower

請從上次中斷的位置繼續開發。

1.先讀取開發實施計畫、開發進度紀錄、版本狀態與目前程式碼
2.核對最後完成的任務、最後一次測試結果、未完成事項與下一個可執行任務
3.不要重做已完成且測試通過的項目；若發現紀錄與程式狀態不一致，先列出差異並以實際檔案和測試結果為準
4.從第一個未完成且前置條件已滿足的任務接續開發
5.每完成一項就執行測試；失敗就修正並複測，通過後更新進度紀錄並直接進入下一項
6.除非遇到需求衝突、必要資料缺失、權限問題或高風險操作，否則不要詢問是否繼續
7.持續執行到開發實施計畫全部完成或出現真正無法自行排除的阻礙

先用短摘要說明「上次完成到哪裡、現在從哪一項開始」，然後直接續作。`,
    testFixPrompt: `/superpower

請處理目前的測試失敗並持續完成開發計畫。

我提供的測試錯誤、畫面截圖或問題描述：
______________________________

執行規則：
1.先重現問題，保留實際錯誤訊息、失敗測試名稱與觸發條件
2.判斷問題位於功能程式、測試程式、資料或設定、介面互動，或需求文件不一致
3.說明根因與預計修改位置，只做解決根因所需的最小修改，不擴充未要求的功能
4.修改後先跑直接相關測試，再跑受影響範圍的回歸測試
5.如果仍失敗，根據新證據繼續定位、修正與複測，不要只回報失敗就停止
6.禁止刪除測試、關閉檢查、放寬驗收條件或寫死結果來讓測試通過
7.測試通過後，更新開發進度紀錄：根因、修改檔案、測試指令、結果、影響範圍與下一個任務
8.完成修正後直接回到開發實施計畫，繼續下一個未完成任務；只有需求衝突、必要資料缺失、權限問題或高風險操作才暫停

現在直接開始重現、修正與測試。`,
    fixAreas: [
      { title: "功能程式", description: "檢查實作邏輯、資料流、API 與畫面狀態。" },
      { title: "測試程式", description: "確認測試是否正確對齊 PRD 與驗收條件。" },
      { title: "資料與設定", description: "檢查測試資料、環境設定、權限與相依服務。" },
      { title: "文件與進度", description: "同步根因、修改內容、測試結果與下一個任務。" }
    ],
    deliverables: [
      { title: "數據庫", description: "資料結構與必要變更" },
      { title: "業務接口代碼", description: "依 TRD 完成模組接口" },
      { title: "單元測試代碼", description: "驗證模組核心邏輯" },
      { title: "接口測試用例", description: "覆蓋正常與異常情境" },
      { title: "自我核對清單", description: "對照 PRD 驗收標準" }
    ],
    closing: "每個任務都留下可續作的進度檢查點；只要沒有真正阻礙，AI 應依開發實施計畫持續完成下一項，而不是停在單次回報。"
  },
  acceptanceVersionGuide: {
    title: "第九步：驗收迭代（版本管理）",
    lead: "每次驗收修改前先保存完整版本，改得不理想時才能比較差異並回到可用狀態。",
    intro: [
      "實際使用後，你一定會反覆調整功能、規則、參數與畫面。有時新版不如舊版，如果沒有留下歷史版本，就無法安全回復。",
      "不熟悉 Git 時，可以先讓 AI 在工作空間內建立完整版本快照；每次修改建立新版本，不直接覆蓋舊版本。"
    ],
    reasons: [
      { number: "01", title: "保留可用版本", description: "每次調整都有修改前後的完整內容，避免新版失敗後無法返回。" },
      { number: "02", title: "看得懂改了什麼", description: "每個版本記錄時間、變更內容與驗收結論，方便比較與追蹤。" }
    ],
    cycle: [
      { number: "01", title: "操作驗收", description: "依實際使用感受檢查" },
      { number: "02", title: "截圖說明問題", description: "圈出不符合預期的位置" },
      { number: "03", title: "建立完整快照", description: "修改前先產生新版本" },
      { number: "04", title: "修正並複驗", description: "確認新版是否真的更好" },
      { number: "05", title: "保留或回滾", description: "通過就留存，不通過就還原" }
    ],
    snapshotContents: [
      "版本號與版本時間",
      "變更說明與變更人",
      "完整驗收文件",
      "完整參數與配置",
      "完整驗收結論",
      "可回滾識別"
    ],
    commands: [
      "列出全部歷史版本",
      "查看指定版本詳情",
      "比較兩個版本差異",
      "回滾至指定版本"
    ],
    operationPrompts: [
      { number: "01", title: "列出全部歷史版本", field: "listVersionsPrompt" },
      { number: "02", title: "查看指定版本詳情", field: "versionDetailPrompt" },
      { number: "03", title: "比較兩個版本差異", field: "compareVersionsPrompt" },
      { number: "04", title: "回滾至指定版本", field: "rollbackVersionPrompt" }
    ],
    listVersionsPrompt: `請以唯讀方式檢查目前專案工作空間的版本紀錄與完整快照；若專案有使用 Git，也一併讀取可用的提交紀錄。

請列出全部歷史版本，依時間由新到舊整理成 Markdown 表格，欄位包含：版本號、建立時間、變更摘要、保存位置、驗收狀態、是否可回滾。

規則：
1. 不修改、覆蓋、刪除或回滾任何檔案。
2. 無法確認的資料標記【待確認】，不要自行猜測。
3. 若工作空間同時存在快照版本與 Git 版本，請分開列出並說明兩者關係。
4. 最後告訴我可用於「查看版本詳情」或「比較版本差異」的版本號。`,
    versionDetailPrompt: `請以唯讀方式查看指定版本的完整內容。

指定版本號：____________________

請輸出：
1. 版本號、建立時間、變更人與變更原因。
2. 此版本包含的完整文件、程式、參數、設定與驗收結論。
3. 相較前一版本新增、修改與移除的內容。
4. 已完成的測試、驗收結果、已知問題與限制。
5. 快照是否完整、是否具備安全回滾條件。

只做檢查與說明，不修改任何檔案；找不到指定版本時，列出可用版本號讓我重新選擇。`,
    compareVersionsPrompt: `請以唯讀方式比較兩個指定版本的差異。

版本 A：____________________
版本 B：____________________

請使用 Markdown 表格整理：檔案新增／修改／移除、需求與規則差異、設定與資料結構差異、程式功能差異、測試與驗收結果差異。

另外說明：
1. 差異對使用者與既有資料的影響。
2. 各版本的已知問題、風險與可回滾狀態。
3. 若要採用其中一版，仍需確認的事項。

不要自動合併、覆蓋、刪除或回滾；無法確認的內容標記【待確認】。`,
    rollbackVersionPrompt: `我要評估回滾至指定歷史版本。

目標版本號：____________________

請依序執行：
1. 先以唯讀方式確認目標版本存在、快照完整且可讀取。
2. 列出回滾會覆蓋或改變的文件、程式、設定、資料與可能影響。
3. 在任何回滾前，先建立目前工作狀態的完整新快照，提供版本號與保存位置。
4. 輸出「回滾前檢查摘要」與實際執行步驟，等待我明確確認。
5. 未收到我的確認前，不得修改、覆蓋、刪除或移動任何現有內容。
6. 確認後才回滾；保留全部歷史版本，不刪除目標版本以外的紀錄。
7. 回滾完成後執行必要測試與驗收，回報恢復版本、測試結果、未解問題與再次回復方式。

若目標版本不完整或可能造成資料遺失，停止操作並標記【需要確認】。`,
    prompt: `你是作為資深的開發工程師、專案經理，具備完整版本管理能力
在獨立工作空間內，對所有驗收相關內容全部做版本化管理
迭代規則：
-在產品驗收全過程中，每發生一次調整、修改、變更(需求變更、文檔修改、參數調整、規則改動、菸收結論修改)，都必須產生一個新版本
-每個版本都需要完整保存時刻下全部完整內容，不允許只保存變更片斷，版本內容包含：版本號、版本時間、變更說明、變更人、完整的驗收全部文檔/參數/配置/結論
-所有版本持久保存在目前工作空間，完整保存歷史快照，不丟失歷史紀錄
-支持隨時執行版本恢復：當我指定某個歷史版本號，你需要完整還原該版本當時的全部內容，回到該版本的工作狀態
-每次升成新版本後，輸出版本摘要：版本編號、變更簡述、可回滾標識
-可以响應我的指令：列出全部理歷史版本、查看某版本詳情、回滾至指定版本、對比兩個版本差異
-禁止直接覆蓋舊版本，所有修改必須通過新建版本完成，原始歷史版本不可被修改、刪除`,
    packagePrompt: "幫我把開發好的應用打包放在：____________________（填寫電腦硬碟資料夾，或指定 GitHub 儲存庫）",
    backupNote: "工作空間內的版本快照仍在同一台電腦，不等於外部備份。重要版本應再複製到另一個磁碟或已確認權限與敏感資料範圍的 GitHub 儲存庫。",
    closing: "驗收標準是實際使用時符合你的需求。發現不對就截圖說明並建立新版本，反覆調整到滿意後，再把最終版本打包到你指定的位置。"
  },
  releaseUseGuide: {
    title: "第十步：發布使用（版本發布）",
    lead: "把驗收完成的版本放到實際裝置，確認能開啟、能操作，也知道下一次要從哪裡進入。",
    intro: [
      "如果只是自己使用，可以把完整專案資料夾複製到手機，在檔案管理器中開啟 HTML 首頁，再將頁面加入書籤。",
      "但手機直接開啟本機 HTML 的支援會因系統、瀏覽器與功能而不同；如果網站需要網路請求、登入、資料同步或離線安裝，使用 HTTPS 網址會更穩定。"
    ],
    options: [
      {
        number: "01",
        title: "手機直接開 HTML",
        bestFor: "純靜態、自己使用、沒有後端服務",
        result: "手機內可直接開啟的本機網頁",
        steps: ["複製完整專案資料夾", "用檔案管理器開啟 index.html", "確認 CSS、JavaScript 與圖片正常", "加入瀏覽器書籤"],
        note: "不同手機可能限制 file:// 頁面功能；不能只複製單一 HTML。"
      },
      {
        number: "02",
        title: "使用公開網址",
        bestFor: "跨裝置使用、需要分享或穩定更新",
        result: "可直接收藏與分享的 HTTPS 網址",
        steps: ["選擇 GitHub Pages 或網站主機", "上傳已驗收版本", "檢查公開範圍與敏感資料", "用手機實際開啟並收藏"],
        note: "通常需要網路；更新網站前仍要保存上一個可回滾版本。"
      },
      {
        number: "03",
        title: "安裝成類 App（PWA）",
        bestFor: "想加入手機主畫面、需要較完整 App 體驗",
        result: "可從手機主畫面啟動的網站應用",
        steps: ["使用 HTTPS 網址", "加入 manifest 與應用圖示", "設定 service worker", "測試安裝、更新與離線行為"],
        note: "PWA 支援與安裝方式依 iOS、Android 和瀏覽器而異。"
      }
    ],
    personalSteps: [
      { number: "01", title: "複製完整資料夾", description: "HTML、CSS、JavaScript、圖片與資料檔一起保留原結構。" },
      { number: "02", title: "點擊首頁檔案", description: "在檔案管理器開啟 index.html 或專案指定首頁。" },
      { number: "03", title: "實機操作確認", description: "逐頁檢查按鈕、資料保存、圖片與離線限制。" },
      { number: "04", title: "收藏到書籤", description: "下次從書籤快速開啟，不必重新找檔案。" },
      { number: "05", title: "需要時升級 PWA", description: "若要主畫面圖示與類 App 體驗，再規劃 PWA。" }
    ],
    checklist: [
      "發布內容是已驗收版本",
      "發布前已保存可回滾版本",
      "手機或目標裝置可正常開啟",
      "主要流程與資料保存已實測",
      "知道網址、檔案或圖示從哪裡進入",
      "已記錄備份位置與後續維護方式"
    ],
    sourceNote: "本步驟沒有提供原文提示詞，因此以操作流程、方案差異與發布檢查表呈現。",
    closing: "發布不是只把檔案搬過去；要在實際裝置完成一次開啟與操作驗收。收藏到書籤或安裝到主畫面後，才算真正可以日常使用。"
  },
  sections: [
    { id: "prep", number: "01", label: "專案準備", short: "確認專案位置", page: "step-1-project-preparation.html" },
    { id: "product", number: "02", label: "產品定義", short: "釐清需求與功能", page: "step-2-product-definition.html" },
    { id: "design", number: "03", label: "原型與設計", short: "先看見操作流程", page: "step-3-prototype-design.html" },
    { id: "build", number: "04", label: "技術與開發", short: "規劃、開發與測試", page: "step-4-technology-development.html" },
    { id: "release", number: "05", label: "驗收與發布", short: "確認版本並使用", page: "step-5-acceptance-release.html" }
  ],
  flowStages: [
    { id: "stage-1", number: "01", title: "第一階段", label: "想法與需求", description: "把想法說清楚", steps: ["0", "1", "2", "3"] },
    { id: "stage-2", number: "02", title: "第二階段", label: "原型與技術", description: "把規格變成施工圖", steps: ["4", "5", "6", "7"] },
    { id: "stage-3", number: "03", title: "第三階段", label: "開發、驗收與發布", description: "做出、驗收並發布", steps: ["8", "9", "10"] }
  ],
  items: [
    { id: "prep", step: "0", section: "prep", name: "建立 AI 工作空間", purpose: "先確認專案資料夾要放在哪裡", push: "準備開始專案，但還沒有決定資料夾要放在哪裡。", materials: ["專案名稱或暫定名稱", "希望存放的磁碟與資料夾位置", "是否需要同步雲端或版本庫（可選）"], result: "確認的專案資料夾路徑，以及後續文件的存放位置。", promptAvailable: false },
    { id: "requirements", step: "1", section: "product", name: "需求收集", purpose: "把模糊想法問清楚", push: "只知道大方向，還說不清楚用戶、痛點、場景與功能。", materials: ["App 名稱或暫定名稱", "初步想法", "已知的使用對象或情境"], result: "使用者需求、使用場景、功能方向、限制條件與可確認的需求文件。", promptAvailable: true },
    { id: "features", step: "2", section: "product", name: "功能清單", purpose: "把產品能力拆成三層", push: "需求已大致清楚，需要知道產品到底要做哪些能力。", materials: ["已確認的需求文件", "使用者場景與痛點", "核心與非核心的取捨"], result: "模組、功能、子功能清單，並區分必做、迭代與未來規劃。", promptAvailable: true },
    { id: "prd", step: "3", section: "product", name: "產品需求文件 PRD", purpose: "定義功能如何運作", push: "功能清單已定，需要交給設計、開發與測試共同理解。", materials: ["完整功能清單", "使用場景與流程", "權限、限制與上線條件"], result: "包含輸入、操作、輸出、規則、異常、權限與驗收條件的 PRD。", promptAvailable: true },
    { id: "prototype", step: "4", section: "design", name: "產品原型", purpose: "先把頁面與互動走一遍", push: "PRD 是文字，想在寫程式前先實際點擊與驗收。", materials: ["已確認 PRD", "頁面或功能清單", "對操作流程的修正意見"], result: "可點擊的頁面原型、彈窗、表單、導覽與互動流程。", promptAvailable: true },
    { id: "ui", step: "5", section: "design", name: "UI 介面設計", purpose: "建立或修改一致的視覺介面", push: "原型已確認，或已有介面需要調整風格、元件與響應式細節。", materials: ["已驗收原型或目前網站", "PRD、品牌資料與現有 UI 程式碼", "想保留與想修改的畫面、截圖或風格描述"], result: "UI 視覺規格、元件規則、可實作變更清單與不同螢幕尺寸的驗收標準。", promptAvailable: true },
    { id: "trd", step: "6", section: "build", name: "技術開發文件 TRD", purpose: "先畫好系統施工圖", push: "PRD、原型與 UI 已定，需要讓開發知道資料、模組與接口如何連接。", materials: ["PRD、原型、UI 設計", "已知的技術限制", "現有系統或外部服務資料"], result: "架構、資料模型、API、權限、安全、依賴、風險與測試要點。", promptAvailable: true },
    { id: "plan", step: "7", section: "build", name: "開發計畫", purpose: "把大工程拆成可控任務", push: "技術方向已清楚，需要知道先做什麼、誰做、如何驗收。", materials: ["PRD 與 TRD", "預計迭代週期", "可投入的人力與時間"], result: "迭代、里程碑、前後端與測試任務、依賴關係及驗收順序。", promptAvailable: true },
    { id: "development", step: "8", section: "build", name: "產品開發與測試", purpose: "依文件逐步做出可運作版本", push: "需求、設計、技術文件與任務順序都已確認。", materials: ["PRD、原型、UI、TRD、開發計畫", "專案工作空間", "每個模組的驗收標準"], result: "可運作的系統、資料與接口、測試紀錄與自我核對清單。", promptAvailable: true },
    { id: "acceptance", step: "9", section: "release", name: "驗收與版本管理", purpose: "反覆修改仍能比較與回復", push: "已有可操作版本，需要依實際使用感受迭代，不讓改版失去控制。", materials: ["目前版本", "驗收清單與截圖", "問題、修改要求與版本命名規則"], result: "驗收紀錄、問題清單、新版本快照、變更摘要與可回滾版本。", promptAvailable: true },
    { id: "release", step: "10", section: "release", name: "發布與使用", purpose: "把完成版本放到實際裝置使用", push: "最終版本已通過驗收，需要打包、備份或放到指定位置。", materials: ["已驗收版本", "發布位置或裝置", "備份、網址、上架或安裝需求"], result: "可使用的發布版本、打包位置、操作方式、備份紀錄與後續維護清單。", promptAvailable: false }
  ]
};
